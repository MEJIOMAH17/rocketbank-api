...
final class GapWorker implements Runnable {
    ...
    static Comparator<Task> sTaskComparator = new C02521();
    ...
    static class C02521 implements Comparator<Task> {
        C02521() {
        }
    ...
    private void buildTaskList() {
        ...
        while (i < size) {
            ...
            if (recyclerView.getWindowVisibility() == 0) {
                ...
                while (true) {
                    boolean z = true;
                    if (i2 >= (layoutPrefetchRegistryImpl.mCount << 1)) {
                        break;
                    }
                    Task task;
    ...
    private void prefetchInnerRecyclerViewWithDeadline(RecyclerView recyclerView, long j) {
        if (recyclerView != null) {
            ...
            if (layoutPrefetchRegistryImpl.mCount != 0) {
                try {
                    String str = "RV Nested Prefetch";
                    if (VERSION.SDK_INT >= 18) {
                        Trace.beginSection(str);
                    }
                    recyclerView.mState.prepareForNestedPrefetch(recyclerView.mAdapter);
                    for (int i = 0; i < (layoutPrefetchRegistryImpl.mCount << 1); i += 2) {
